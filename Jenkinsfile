node {
    withCredentials([[$class: 'UsernamePasswordMultiBinding',
        ...
        stage('Pull') {
            try {
                git branch: 'main', url: 'https://github.com/ljh4770/fisa05-jenkins-practice.git'
        ...
        stage('Deploy') {
            sshagent(credentials: ['ec2-fastapi-server']) {
                ...
                if (env.BUILD_NUMBER.toInteger() != 1) {
                    sh(script: 'ssh -o StrictHostKeyChecking=no ubuntu@43.200.225.220 "sudo docker rm -f docker-fastapi"')
                } else {
                    echo "Initial run (BUILD_NUMBER=${env.BUILD_NUMBER}). Skipping docker container removal."
                }
                // sh(script: 'ssh -o StrictHostKeyChecking=no ubuntu@43.200.225.220 "pwd"')
                sh(script: 'ssh -o StrictHostKeyChecking=no ubuntu@43.200.225.220 "sudo docker run --name docker-fastapi --env-file .env -e TZ=Asia/Seoul -p 80:8000 -d -t ${DOCKER_USER_ID}/fisa-app:${BUILD_NUMBER}"')
            }
